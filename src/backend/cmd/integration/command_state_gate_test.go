...
func TestCommandStateGate_OptimisticDoesNotClearAndSyncedIsGated(t *testing.T) {
	g := &commandStateGate{
	...
	// Optimistic publishes should always be allowed and must not clear the gate.
	if !g.allow(deviceID, map[string]any{"power": "on"}, true) {
		t.Fatalf("expected optimistic publish to be allowed")
	}

	// Synced publish should be blocked while the expected state hasn't arrived.
	if g.allow(deviceID, map[string]any{"power": "off"}, false) {
	...
	if !g.allow(deviceID, map[string]any{"power": "on"}, false) {
